...
def train_ppo(
    ...
    with tqdm(range(start_step, steps + start_step)) as pbar:
        for step in pbar:
            ...
            # action = strategy.select_action(policy.detach().cpu().numpy())
            pi_old = policy[action].item()
            ...
            episode_steps += 1
            count += 1
            replay_memory.push(state, action, reward, next_state, done, pi_old)
            ...
            if batch is not None:
                states, actions, rewards, next_states, dones, pi_olds = batch
                states = np.array(states)
                actions = np.array(actions)
                rewards = np.array(rewards)
                next_states = np.array(next_states)
                dones = np.array(dones)
                pi_olds = np.array(pi_olds)
                ...
                pi_olds = torch.tensor(pi_olds).float().to(device)
                ...
                probs = actor_model(states).gather(1, actions).squeeze()
                trust_ratio = probs / pi_olds
                values = critic_model(states).squeeze()
                advantages = labels - values
                actor_loss = -(torch.min(
                    trust_ratio * advantages, 
                    torch.clamp(trust_ratio, 1 - ppo_clip, 1 + ppo_clip) * advantages
                )).mean()
